...
class MapPage : ComponentActivity() {
    ...
    private fun navigateToQRCodeScanner(polygonId: String) {
        val intent = Intent(this, QRCodeScannerActivity::class.java).apply {
            putExtra("POLYGON_ID", polygonId)
        }
        startActivity(intent)
    }
}
...
fun MainMap(onCartClick: () -> Unit,onProfileClick: () -> Unit, onQRCodeScan: (String) -> Unit){
    ...
    selectedMarkerData?.let { markerData ->
        LocationInfoDialog(
            ...
            onQRCodeClick = {
                onQRCodeScan(markerData.id)
                selectedMarkerData = null
                polygonData = polygonData.filterNot { it.id == markerData.id }
